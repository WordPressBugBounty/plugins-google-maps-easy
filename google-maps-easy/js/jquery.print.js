...
(function ($) {
  ...
  function getjQueryObject(string) {
    ...
    var jqObj = jQuery('');
    try {
      jqObj = jQuery(string).clone();
    } catch (e) {
      jqObj = jQuery('<span />').html(string);
    }
    return jqObj;
  }
  ...
  function printContentInIFrame(content, options) {
    ...
    if (iframeCount === 0) {
      ...
      $iframe = jQuery('<iframe height="0" width="0" border="0" wmode="Opaque"/>').prependTo('body').css({
        position: 'absolute',
        top: -999,
        left: -999,
      });
