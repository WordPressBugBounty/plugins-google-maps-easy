...
class fieldAdapterGmp
{
  ...
  public static function taxDataHtml($field)
  {
    ...
    return '<div>' .
      __('Apply To', GMP_LANG_CODE) .
      '
            <div id="tax_address">
                <b>' .
      __('Address', GMP_LANG_CODE) .
      '</b><br />
                ' .
      __('Destination', GMP_LANG_CODE) .
      ':' .
      htmlGmp::checkboxlist('params[dest]', ['optionsGmp' => $listOfDest]) .
      '<br />
                ' .
      __('Country', GMP_LANG_CODE) .
      ':' .
      htmlGmp::countryList('params[country]', ['notSelected' => true, 'value' => $field->value['country']]) .
      '<br />
            </div>
            <div id="tax_category">
                <b>' .
      __('Categories', GMP_LANG_CODE) .
      '</b><br />
                ' .
      (empty($cOptions) ? __('You have no categories', GMP_LANG_CODE) : htmlGmp::checkboxlist('params[categories][]', ['optionsGmp' => $cOptions])) .
      '<br />
                    <b>' .
      __('Brands', GMP_LANG_CODE) .
      '</b><br />
                ' .
      (empty($bOptions) ? __('You have no brands', GMP_LANG_CODE) : htmlGmp::checkboxlist('params[brands][]', ['optionsGmp' => $bOptions])) .
      '<br />
            </div>
            <div>' .
      __('Tax Rate', GMP_LANG_CODE) .
      ': ' .
      htmlGmp::text('params[rate]', ['value' => $field->value['rate']]) .
      '</div>
            <div>' .
      __('Absolute', GMP_LANG_CODE) .
      ': ' .
      htmlGmp::checkbox('params[absolute]', ['checked' => $field->value['absolute']]) .
      '</div>
        </div>';
  }
  ...
  public static function getFieldAttributes($params)
  {
    ...
    if (!empty($params->attr)) {
      foreach ($params->attr as $key => $value) {
        $output .= __($key) . ':<br />';
        $output .= htmlGmp::text('params[attr][' . $key . ']', ['value' => $value]) . '<br />';
      }
    } else {
      $output .= __('class', GMP_LANG_CODE) . ':<br />';
      $output .= htmlGmp::text('params[attr][class]', ['value' => '']) . '<br />';
      $output .= __('id', GMP_LANG_CODE) . ':<br />';
      $output .= htmlGmp::text('params[attr][id]', ['value' => '']) . '<br />';
    }
    return $output;
  }
  /**
  ...
  public static function getCachedStates($clearCache = false)
  {
    if (empty(self::$states) || $clearCache) {
      self::$states = frameGmp::_()
        ->getTable('states')
        ->leftJoin(frameGmp::_()->getTable('countries'), 'country_id')->getAll('toe_states.id,
					toe_states.name, 
					toe_states.code, 
