...
class fieldGmp
{
   ...
   */
  public $adapt = ['htmlGmp' => '', 'dbFrom' => '', 'dbTo' => ''];
  /**
    ...
    if (method_exists('htmlGmp', $this->html)) {
      ...
      if ($this->name == 'default_value') {
        ...
        if (!empty($optionsFromDb)) {
          $options = [0 => __('Select', GMP_LANG_CODE)];
  ...
  public function viewField($name, $fieldValue = '')
  {
    ...
    $params = ['optionsGmp' => $options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked];
    $output = '';
    if (method_exists('htmlGmp', $method)) {
      $output .= htmlGmp::$method($name, $params);
      $output .= htmlGmp::hidden('extra_field[' . $this->name . ']', ['value' => $this->id]);
    }
    return $output;
  }
  ...
  public function processParams($tag, $id)
  {
    ...
    if ($this->name == 'params') {
      ...
      if ($tag > 0 || $id == 0) {
        ...
        $output .= htmlGmp::text('params[width]', ['value' => $width]) . '<br />';
        $output .= __('height', GMP_LANG_CODE) . ':<br />';
        $output .= htmlGmp::text('params[height]', ['value' => $height]) . '<br />';
        $output .= '</div>';
      }
  ...
  public function prepareConfigOptions($xml)
  {
    ...
    foreach ($config->params->param as $param) {
      ...
      $config_params[$name] = ['type' => $type, 'label' => $label, 'helperGmp' => $helper, 'moduleGmp' => $module, 'values' => $values, 'default' => $default, 'description' => $description];
    }
    ...
    if (!empty($configOptions)) {
      ...
      foreach ($params as $param) {
        ...
        foreach ($configOptions as $key => $value) {
          ...
          $htmlParams = ['value' => $fieldValue, 'optionsGmp' => $options];
          if ($method == 'checkbox') {
            $htmlParams['value'] = 1;
