...
class optionsGmp extends moduleGmp
{
  ...
  {
    dispatcherGmp::addAction('afterModulesInit', [$this, 'initAllOptValues']);
    dispatcherGmp::addFilter('mainAdminTabs', [$this, 'addAdminTab']);
  }
  public function addAdminTab($tabs)
  {
    $tabs['settings'] = [
      'label' => __('Settings', GMP_LANG_CODE),
  ...
  {
    if (empty($this->_tabs)) {
      $this->_tabs = dispatcherGmp::applyFilters('mainAdminTabs', [
  ...
  public function getAll()
  {
    if (empty($this->_options)) {
      $this->_options = dispatcherGmp::applyFilters('optionsDefine', [
        'general' => [
          'label' => __('General', GMP_LANG_CODE),
          'opts' => [
            'api_domain' => [
              'label' => __('API Domain', GMP_LANG_CODE),
              'desc' => __('Sets domain for google API scripts', GMP_LANG_CODE),
              'def' => '',
              'html' => 'selectbox',
      ...
      foreach ($this->_options as $catKey => $cData) {
        foreach ($cData['opts'] as $optKey => $opt) {
          ...
          if (isset($opt['pro']) && !$isPro) {
            $this->_options[$catKey]['opts'][$optKey]['pro'] = frameGmp::_()
              ->getModule('supsystic_promo')
              ->generateMainLink('utm_source=plugin&utm_medium=' . $optKey . '&utm_campaign=googlemaps');
          }
        }
      }
