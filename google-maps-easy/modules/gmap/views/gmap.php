...
class gmapViewGmp extends viewGmp
{
  ...
  public function drawMap($params)
  {
    ...
    if (empty($mapObj)) {
      return isset($params['id']) ? sprintf(__('Map with ID %d not found', GMP_LANG_CODE), $params['id']) : __('Map not found', GMP_LANG_CODE);
    }
    ...
    if ($mapObj['params']['map_display_mode'] == 'popup') {
      frameGmp::_()->addScript('jquery-ui-dialog', '', ['jquery']);
    ...
    $mapVarTemp = isset($mapObj['params']['markers_list_type']) && !empty($mapObj['params']['markers_list_type']) ? $mapObj['params']['markers_list_type'] : '';
  ...
  public function getEditMap($id = 0)
  {
    ...
    $stylizationsForSelect = ['none' => __('None', GMP_LANG_CODE)];
    ...
    $this->assign(
      ...
      frameGmp::_()
        ...
        ->getModel()
        ->getMarkerGroupsForSelect(['0' => __('None', GMP_LANG_CODE)]),
    );
  ...
  public function connectMapsAssets($params, $forAdminArea = false)
  {
    if (!$forAdminArea && isset($params['is_static']) && (int) $params['is_static']) {
      ...
      if (frameGmp::_()->getModule('supsystic_promo')->isPro() && frameGmp::_()->getModule('add_map_options') && method_exists(frameGmp::_()->getModule('add_map_options'), 'connectStaticMapCore')) {
