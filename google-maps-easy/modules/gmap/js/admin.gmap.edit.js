...
jQuery(document).ready(function () {
      ...
      onSuccess: function (res) {
        if (!res.error) {
          if (res.data.map_id) {
            ...
            jQuery('#gmpMarkersListGrid').jqGrid('setGridParam', { url: newMarkersTblUrl, width: null, shrinkToFit: false });
            ...
            if (GMP_DATA.isPro) {
              ...
              jQuery('#gmpShapesListGrid').jqGrid('setGridParam', { url: newShapesTblUrl, width: null, shrinkToFit: false });
            }
          }
          if (firstTime) {
            gmpCheckShortcode();
  ...
  // Map zoom
  jQuery('#gmpMapForm [name="map_opts[zoom_type]"]')
    .change(function () {
  ...
  jQuery('[name="map_opts[custom_controls_improve_search]"]')
    .on('change', function () {
  ...
  jQuery('#gmpMapForm input[name="map_opts[adapt_map_to_screen_height]"]')
    .change(function () {
      if (parseInt(jQuery(this).val())) {
  ...
  jQuery('#gmpMarkerTab input[name="marker_opts[params][description_mouse_hover]"]').change(function () {
    if (jQuery(this).prop('checked') === true) gmpShowCloseDescriptionCheckbox();
    else gmpHideCloseDescriptionCheckbox();
  });
...
function gmpUpdateClusterIcon(url, width, height) {
  ...
  g_gmpMap.setParam('marker_clasterer_icon', url).setParam('marker_clasterer_icon_width', width).setParam('marker_clasterer_icon_height', height).enableClasterization(g_gmpMap.getParam('marker_clasterer'));
}
...
function _gmpResizeRightSidebar(container) {
  ...
  if (wndWd > 991) {
    newHeight = wndHt - jQuery('#wpadminbar').outerHeight() - jQuery('#gmpMapPreview').outerHeight() - jQuery('.gmpControlBtns:first').outerHeight() - jQuery('.ui-jqgrid-htable:first').outerHeight();
  }
